...
class PokemonManager: NSObject {
    ...
    func getPokemon(completionGetPokemonList : @escaping([Pokemon]) ->()){
        ...
        Helpers.shared.fetchData(url: "https://pokeapi.co/api/v2/pokemon?limit=20", model: PokemonModel.self){ [self] completion in
...
//            let tempcompletiongetPokemon = completion.results
    ...
    func getDetailPokemon(id: Int, completionGetPokemonDetail: @escaping(PokemonDetailModel) -> ()){
        ...
        Helpers.shared.fetchData(url: "https://pokeapi.co/api/v2/pokemon/\(id)/", model: PokemonDetailModel.self){ [self] completion in
